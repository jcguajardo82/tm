...
function vanillaSelectBox(domSelector, options) {
    ...
    this.userOptions = {
        ...
        minWidth: -1,
        maxHeight: 400,
        translations: { "all": "All", "items": "Seleccionados", "selectAll": "Select All", "clearAll": "Clear All" },
        search: false,
        placeHolder: "",
        stayOpen: false,
    ...
    this.createTree = function () {
        ...
        if (self.multipleSize != -1) {
            if (nrActives > self.multipleSize) {
                let wordForItems = self.userOptions.translations.items || "Seleccionados"
            ...
            if (!self.isMultiple) {
                ...
                for (let i = 0; i < self.options.length; i++) {
                    ...
                    if (self.options[i].selected) {
                        nrActives++;
                        selectedTexts += sep + self.options[i].value;
                        sep = ",";
                    }
                }
                if (nrAll == nrActives) {
            ...
            if (vanillaSelectBox_type(values) == "array") {
                ...
                Array.prototype.slice.call(listElements).forEach(function (x) {
                    ...
                    if (foundValues.indexOf(x.getAttribute("data-value")) != -1) {
                        ...
                        nrActives++;
                        selectedTexts += sep + x.getAttribute("data-value");
                        sep = ",";
                    } else {
                        x.classList.remove("active");
                    }
